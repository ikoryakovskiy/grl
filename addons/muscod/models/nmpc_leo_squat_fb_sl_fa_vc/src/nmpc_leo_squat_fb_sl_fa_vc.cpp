...
const int NXD  = 2*(3);  // Number of differential states
const int NXA  = 0;  // Number of algebraic states

const int NU   = (3);  // Number of controls
...
void lsqfcn_height_tracking (
  ...
  res[res_cnt++] = 300.00 * (position_root[2] - p[parameter["h_ref"]]);

	// track: || com_x,y - support center_x,y ||_2^2
  res[res_cnt++] = 500.00 * (position_CoM[0] - suppport_center[0]);
